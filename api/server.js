...
async function initDatabase() {
  ...
  try {
    ...
    // Membros com Acesso Exclusivo (Gerados pelo Admin)
    await client.query(`
    ...
    // Visitantes (Registos Públicos - Apenas Identificação)
    await client.query(`
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // UPDATE SYSTEM CONFIG (Corrigido)
    ...
    // REGISTO PÚBLICO (VAI PARA TABELA VISITORS)
    ...
    if (path === '/api/admin/users/create' && method === 'POST') {
      ...
      if (check.rows.length > 0) return res.status(400).json({ error: 'ID_EXISTENTE' });
    ...
    if (path === '/api/login' && method === 'POST') {
      ...
      const normalizedUsername = username.toLowerCase().trim();
      const result = await pool.query('SELECT * FROM managed_users WHERE username = $1 AND password = $2', [normalizedUsername, pass]);
    ...
    // OUTRAS ROTAS ADMIN (DELETE/STATUS)
