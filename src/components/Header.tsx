          ...
          <div className="hidden md:flex items-center space-x-1">
            ...
            {user ? (
              <div className="flex items-center ml-4 space-x-4">
                ...
                {!user.hasLiveAccess && user.role !== 'admin' && (
                  <Link
                    to="/login"
                    className="px-4 py-1.5 text-xs font-bold text-ministry-blue border border-ministry-gold bg-amber-50/70 hover:bg-ministry-gold hover:text-white rounded-xl transition flex items-center space-x-1.5 shadow-sm"
                    title="Entrar com credenciais de Membro / Parceiro"
                  >
                    <Lock size={13} className="text-ministry-gold group-hover:text-white" />
      ...
      {isOpen && (
        <div className="md:hidden bg-white border-t border-gray-100 pb-4 shadow-lg">
          ...
          <div className="px-4 pt-4 border-t border-gray-100 space-y-3">
            {user && !user.hasLiveAccess && user.role !== 'admin' && (
              <Link 
                to="/login" 
                onClick={() => setIsOpen(false)}
                className="w-full py-2.5 px-4 text-center block text-ministry-blue font-bold border border-ministry-gold bg-amber-50/70 rounded-xl text-xs uppercase tracking-wider"
              >
                {t('common.login')} ({t('welcome.partner')})
              </Link>
            )}
            {user ? (
              ...
              <div className="grid grid-cols-2 gap-4">
                <Link to="/login" onClick={() => setIsOpen(false)} className="px-4 py-2 text-center text-ministry-blue font-bold border border-gray-100 rounded-lg">{t('common.login')}</Link>
                <Link to="/register" onClick={() => setIsOpen(false)} className="px-4 py-2 text-center bg-ministry-blue text-white rounded-lg font-bold">{t('common.register')}</Link>
              </div>
            )}
          </div>
        </div>
