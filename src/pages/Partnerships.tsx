...
const Partnerships: React.FC = () => {
  ...
  const [selectedBranches, setSelectedBranches] = useState<string[]>(
    location.state?.showForm && location.state?.selectedBranches 
      ? location.state.selectedBranches 
      : (location.state?.showForm ? [PARTNERSHIP_BRANCHES[0].id] : [])
  );
  ...
  const handleSponsorClick = () => {
    if (selectedBranches.length === 0) {
      alert(t('partnerships.error_select_branch'));
      return;
    }
    setShowForm(true);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await api.payments.process({
        userId: user?.id || 'anon',
        userName: user?.fullName || t('common.anonymous'),
        amount: parseFloat(amount),
        method: method,
        type: 'partnership',
        description: t('common.partnership_seed', { branches: selectedBranches.map(id => t(PARTNERSHIP_BRANCHES.find(b => b.id === id)?.name || '')).join(', ') })
      });

      setStep(3);
    } catch (e: any) {
      alert(e.message || t('common.error_payment'));
    } finally {
    ...
    <div className="bg-white min-h-screen">
      ...
      <div className="bg-ministry-blue py-32 text-center relative overflow-hidden">
        <div className="max-w-4xl mx-auto px-4 relative z-10 animate-in fade-in zoom-in duration-700">
          <h1 className="text-5xl md:text-7xl font-display font-black text-white mb-8 tracking-tight uppercase">{t('partnerships.hero_title')}</h1>
          <p className="text-2xl text-blue-100/70 font-light max-w-3xl mx-auto leading-relaxed">
            {t('partnerships.hero_subtitle')}
          </p>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-16">
          ...
          <div className="lg:col-span-2 space-y-12">
            <div>
              <h2 className="text-4xl font-display font-black text-ministry-blue mb-12 uppercase tracking-tighter">{t('partnerships.branches_title')}</h2>
                  ...
                  >
                    ...
                    <div className="p-8">
                      <div className="flex justify-between items-start mb-4">
                        <h3 className="text-2xl font-black text-ministry-blue uppercase tracking-tight">{t(branch.name)}</h3>
                      ...
                      <p className="text-gray-500 text-sm mb-6 leading-relaxed line-clamp-3 font-medium">{t(branch.description)}</p>
                      <div className="flex items-center text-[10px] text-ministry-gold font-black uppercase tracking-widest bg-ministry-gold/5 p-3 rounded-xl border border-ministry-gold/10">
                        ...
                        {t('partnerships.impact_label')}: {t(branch.impact)}
                      </div>
                    </div>
                  </div>
                ))}
            ...
            <div className="sticky top-32 space-y-8">
              {!showForm ? (
                <div className="bg-gray-50 border border-gray-100 p-10 rounded-[3rem] shadow-xl">
                  <h3 className="text-3xl font-display font-black text-ministry-blue mb-8 uppercase tracking-tighter">{t('partnerships.sidebar_title')}</h3>
                  <div className="space-y-6 mb-10">
                    <p className="text-gray-500 font-medium leading-relaxed">{t('partnerships.sidebar_desc')}</p>
                  ...
                  >
                    <span>{t('partnerships.btn_proceed')}</span>
                ...
                <div className="bg-white border-[6px] border-ministry-gold p-10 rounded-[3rem] shadow-[0_35px_60px_-15px_rgba(197,160,89,0.2)] animate-in zoom-in duration-500">
                  {step === 1 && (
                    <div>
                      <h3 className="text-3xl font-display font-black text-ministry-blue mb-8 uppercase tracking-tighter">{t('partnerships.step_1_title')}</h3>
                      <form className="space-y-6" onSubmit={(e) => { e.preventDefault(); setStep(2); }}>
                        <div>
                          <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-3 ml-2">{t('partnerships.full_name')}</label>
                          <input
                            type="text"
                        ...
                        <div>
                          <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-3 ml-2">{t('partnerships.amount_label')}</label>
                          <input
                            type="number"
                        ...
                        <div>
                          <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-3 ml-2">{t('partnerships.prayer_request')}</label>
                          <textarea
                            className="w-full bg-gray-50 border-0 rounded-2xl px-6 py-4 focus:ring-4 focus:ring-ministry-gold/20 font-medium"
                            rows={3}
                            placeholder={t('partnerships.prayer_placeholder')}
                          ></textarea>
                        </div>
                  ...
                  {step === 2 && (
                    <div className="animate-in slide-in-from-right duration-500">
                      <h3 className="text-3xl font-display font-black text-ministry-blue mb-8 uppercase tracking-tighter">{t('partnerships.step_2_title')}</h3>
                      <div className="space-y-8">
                        <div>
                          <p className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] mb-4 ml-2">{t('partnerships.local_methods')}</p>
                            ...
                            {PAYMENT_METHODS.angolan.map(pm => (
                              ...
                              >
                                <span className="flex items-center space-x-3">
                                  ...
                                  <span className="text-sm font-black text-ministry-blue">{t(pm.name)}</span>
                                </span>
                        ...
                        <div>
                          <p className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] mb-4 ml-2">{t('partnerships.intl_methods')}</p>
                          <div className="grid grid-cols-1 gap-3">
                        ...
                        <div className="flex space-x-4 pt-4">
                          <button onClick={() => setStep(1)} disabled={isProcessing} className="flex-1 py-6 text-gray-400 font-black uppercase tracking-widest text-xs hover:text-gray-600 transition">{t('partnerships.btn_back')}</button>
                          <button
                          ...
                          >
                            {isProcessing ? <Loader2 className="animate-spin mx-auto" /> : t('partnerships.btn_finish')}
                          </button>
                        </div>
                      </div>
                    </div>
                  ...
                  {step === 3 && (
                    <div className="text-center py-10 animate-in zoom-in duration-500">
                      ...
                      <h3 className="text-3xl font-display font-black text-ministry-blue mb-4 uppercase tracking-tighter">{t('partnerships.step_3_title')}</h3>
                      <p className="text-gray-500 mb-10 font-medium leading-relaxed">{t('partnerships.step_3_desc')}</p>
                      <button
