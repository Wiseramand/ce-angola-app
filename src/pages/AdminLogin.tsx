    ...
    <div className="min-h-screen bg-[#050a15] flex items-center justify-center p-6">
      <div className="max-w-md w-full">
        <div className="text-center mb-10">
          ...
          <h1 className="text-2xl font-display font-bold text-white tracking-tight">{t('auth.admin_login_title')}</h1>
          <p className="text-gray-500 text-sm mt-2 font-medium">{t('auth.admin_login_subtitle')}</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="bg-gray-900/50 backdrop-blur-xl p-10 rounded-[3rem] border border-white/5 shadow-2xl space-y-6">
          ...
          <div>
            <label className="block text-[10px] font-black text-gray-500 uppercase mb-2 tracking-[0.2em] ml-2">{t('auth.admin_id')}</label>
          ...
          <div>
            <label className="block text-[10px] font-black text-gray-500 uppercase mb-2 tracking-[0.2em] ml-2">{t('auth.password')}</label>
            <div className="relative">
          ...
          >
            {isLoading ? <Loader2 className="animate-spin mx-auto" size={20} /> : t('auth.admin_unlock')}
          </button>
        </form>

        <p className="text-center mt-10 text-[10px] text-gray-700 font-bold uppercase tracking-widest">
          {t('auth.admin_security_note')} • v4.0.1
        </p>
      </div>
    </div>
  );
